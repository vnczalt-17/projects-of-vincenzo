...
def main_get():
    return render_template('index.html', not_valid=False, developer_name='İrfan İskender')

@app.route('/', methods=['POST'])
def main_post():
    alpha = request.form['number']
    if not alpha.isdecimal():
        return render_template('index.html', not_valid=True, developer_name='İrfan İskender')
    number=int(alpha)
    if not (0 < number):
        return render_template('index.html', not_valid=True, developer_name='İrfan İskender')
    return render_template('result.html', milliseconds=number, result=convert(number), developer_name='İrfan İskender')
